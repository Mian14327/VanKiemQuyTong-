...
export function ShieldOrb() {
  ...
  useFrame(({ clock }) => {
    ...
    if (meshRef.current && glowRef.current) {
      ...
      (glowRef.current.material as THREE.MeshBasicMaterial).opacity = newOpacity * 0.5;
      meshRef.current.visible = newOpacity > 0.01;
      glowRef.current.visible = newOpacity > 0.01;

      const currentPos = isTracking ? targetPosition : new THREE.Vector3(Math.sin(time * 0.5) * 6, Math.cos(time * 0.4) * 4, Math.sin(time * 0.3) * 3);
  ...
  const innerMaterial = useMemo(() => new THREE.MeshBasicMaterial({
    color: 0x88ccff, transparent: true, opacity: 0, side: THREE.DoubleSide, blending: THREE.AdditiveBlending,
  }), []);

  const glowMaterial = useMemo(() => new THREE.MeshBasicMaterial({
    color: 0x4488ff, transparent: true, opacity: 0, side: THREE.BackSide, blending: THREE.AdditiveBlending,
  }), []);
