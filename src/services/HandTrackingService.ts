  ...
  initPromise = (async () => {
    ...
    try {
      ...
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { width: { ideal: widthIdeal }, height: { ideal: heightIdeal }, facingMode: 'user' },
      });
...
export function detectGesture(landmarks: { x: number; y: number; z: number }[]): GestureMode {
  const getAngle = (
  ...
  const isFingerExtended = (fingerIdx: number) => {
    let p1, p2, p3;
    if (fingerIdx === 0) { p1 = landmarks[1]; p2 = landmarks[2]; p3 = landmarks[3]; }
    else { const o = fingerIdx * 4; p1 = landmarks[o + 1]; p2 = landmarks[o + 2]; p3 = landmarks[o + 3]; }
    return getAngle(p1, p2, p3) > 150;
  };

  const f = [0, 1, 2, 3, 4].map(isFingerExtended);
  const [thumb, index, middle, ring, pinky] = f;

  if (f.filter((x) => x).length <= 1) return 'SHIELD';
  if (index && pinky && !middle && !ring) return 'DAGENG';
  if (thumb && index && !middle && !ring && !pinky) return 'DRAGON';
  if (index && middle && !pinky) return 'DRAGON';
